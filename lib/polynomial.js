...
class Polynomial {
   ...
   * @return {number} The order of the polynomial.
   */
  get order() {
    // Take maximum with 0 so that emtpy polynomials return 0.
    return Math.max(this.coefficients.length - 1, 0)
  }
  ...
  /**
   * Interpolates a list of points from a polynomial at the point 0.
   * @param  {Array} xs A list of x values from the polynomial.
   * @param  {Array} ys A list of y values from the polynomial.
   * @return {Number}    The constant of the polynomial.
   */
  static interpolate(xs, ys) {
    return everpolate.polynomial(0, xs, ys)[0]
  }
}
